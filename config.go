...
func (c *Config) GetInitialFields() map[string]interface{} {
...
func NewConfigFromInterface(c ConfigInterface) *Config {
	dft := NewDefaultConfig()
	if c.GetLevel() != "" {
		if lvl, err := ParseLevel(c.GetLevel()); err == nil {
			dft.Level = lvl
		}
	}
	if c.GetEncoding() != "" {
		dft.Encoding = c.GetEncoding()
	}
	if c.GetOutput() != "" {
		dft.OutputPaths = []string{c.GetOutput()}
	}
	dft.DisableStacktrace = c.GetDisableStacktrace()
	dft.InitialFields = c.GetInitialFields()
	return dft
}
